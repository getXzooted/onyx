...
function onyx_deploy() {
    ...
    # 1. Only proceed if we aren't already running from the target directory
    if [[ "$CURRENT_DIR" != "$INSTALL_DIR" ]]; then
        exit 0  
    fi

    # 2. Backup existing config if it exists before copying
    ...
    # 5. Restore the backup config if it existed
    if [ -d $BACKUP_PATH ]; then
